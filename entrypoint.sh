...
then
  ...
  mask_token git remote set-url origin "https://x-access-token:${GITHUB_TOKEN}@github.com/${GITHUB_REPOSITORY}.git"
else
  git clone "$DOWNSTREAM_REPO" --branch ${DOWNSTREAM_BRANCH} work
  cd work || { echo "Missing work dir" && exit 2 ; }
  mask_token git remote set-url origin "https://x-access-token:${GITHUB_TOKEN}@github.com/${DOWNSTREAM_REPO/https:\/\/github.com\//}"
fi
